...
3. Search for `Rhino_JA`, then click `INSTALL`.
...
1. Open `File`->`Examples`->`Rhino_JA`->`RhinoExample`.
...
1. Compile and upload the `Rhino_JA/GetUUID` sketch from the `File -> Examples` menu. Copy the UUID of the board printed at the beginning of the session to the serial monitor.
